...
def compute_assets_correlation(request: ComputeCorrelationRequest = Body(...),
    ...
    """
    Computes correlation among different stocks based on the provided metric and returns the result.
    ...
    try:
        correlation_df = app.compute_assets_correlation(request.stock_ids, request.start_date, request.end_date, request.metric)
